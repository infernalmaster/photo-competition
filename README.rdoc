...
== Console
  ...
  docker exec -it photo-competition_app_1 bundle exec irb -r './application.rb'

  or

  docker exec -it photo-competition_app_1 ./console

== Deploy

  Run it first time to provision server:

  docker-machine create --driver digitalocean --digitalocean-access-token=xx --digitalocean-size=1gb --digitalocean-region=fra1 nazarenko

  Then:

  eval (docker-machine env nazarenko)
